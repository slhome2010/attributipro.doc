...
На выбранном узле нужно войти в режим [редактирования](/module-features/editing.md), после чего ввести необходимые значения через разделитель или без.  

## Выбор из списка {#using-duty-autocomplete}

Существует еще один режим создания **Дежурного шаблона**.  

Если после входа в режим редактирования нажать `Alt + Shift + Click`, то включится подгрузка уже имеющихся в товарах **Значений**. Если таких **Значений** нет, то список не сформируется.  

| ![Duty dropdown select](/img/tutorial/duty_autocomplete.png) |
|:--:|
| *Duty autocomplete* |  

| ![Duty dropdown no data](/img/tutorial/duty_autocomplete_category.png) |
|:--:|
...
Операцию можно производить только на вкладке *`Атрибуты`*. Данная операция не затрагивает товары. Удаляется только **Дежурный шаблон**.  
